...
//		'allowed_symbols'		=> '',
	...
	public function getImage(){
		...
		if (!$this->optinos['width'] && isset($width))
			$this->optinos['width'] = $width;
		if (!$this->optinos['height'] && isset($height))
			$this->optinos['height'] = $height;
		if (!$this->optinos['length'] && isset($length))
			$this->optinos['length'] = $length;
		...
		while(true){
			...
			for($i=0;$i<$this->optinos['length'];$i++){
				...
				$y=mt_rand(-$fluctuation_amplitude, $fluctuation_amplitude)+($this->optinos['height']-$fontfile_height)/2+2;
